$(document).ready(function () {
  ...
  $("nav.header-responsive li.active").addClass("open").children("ul").show();
    ...
    $("nav.header-responsive li.sub-menu > a").on("click", function(e) {
        const rect = this.getBoundingClientRect();
        const beforeWidth = 60;
        if (e.clientX - rect.left < beforeWidth) {
            e.preventDefault();
            const li = $(this).parent("li");
            li.toggleClass("open");
            li.children("ul").slideToggle(400);
            li.siblings("li").removeClass("open").children("ul").slideUp(400);
        }
    });
