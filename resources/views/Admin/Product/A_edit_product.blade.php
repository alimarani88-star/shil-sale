        ...
        <section class="row">
            <div class="col-12">
                <section class="main-body-container">
                    ...
                    <div>
                        <form action="{{ route('A_s_edit_product', $product->id) }}" method="post"
                            ...
                            <div class="row">
                                        ...
                                        <select class="form-control form-control-sm select2"
                                            ...
                                            @foreach($groups['data'] as $group)
                                                <option value="{{ $group['id'] }}"
                                                    {{ old('product_group_id_in_app', $product->group_id_in_app) == $group['id'] ? 'selected' : '' }}>
                                                    {{ $group['title'] }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-12 col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="price_unit" id="price_unit" class="form-control form-control-sm">
                                            <option value="ریال"
                                                    @if(old('price_unit',$product->price_unit) == 'ریال') selected @endif>
                                                ریال
                                            </option>
                                            <option value="تومان"
                                                    @if(old('price_unit',$product->price_unit) == 'تومان') selected @endif>
                                                تومان
                                            </option>
                                            <option value="دلار"
                                                    @if(old('price_unit',$product->price_unit) == 'دلار') selected @endif>
                                                دلار
                                            </option>
                                            <option value="درهم"
                                                    @if(old('price_unit',$product->price_unit) == 'درهم') selected @endif>
                                                درهم
                                            </option>
                                        </select>
                                        @error('price_unit')
                                ...
                                <div class="col-12 col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="sales_start_date" id="sales_start_date"
                                               value="{{ old('sales_start_date', $product->sales_start_date) }}"
                                               class="form-control form-control-sm">
                                ...
                                <div class="col-12 col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="sales_end_date" id="sales_end_date"
                                               value="{{ old('sales_end_date', $product->sales_end_date) }}"
                                               class="form-control form-control-sm">
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                @for($i=1; $i<=4; $i++)
                                                    @php
                                                        $image = $product->images->where('position', $i)->first();
                                                    @endphp
                                                    <div class="col-12 col-md-6 mb-3">
                                                        <div class="form-group">
                                                            ...
                                                            <div id="preview{{ $i }}" class="mt-2 d-flex flex-wrap gap-3">
                                                                @if($image)
                                                                    <div class="preview-item d-flex align-items-center gap-2" id="image-{{ $image->id }}">
                                                                        <img src="{{ url('get_image_by_id/'.$image->id) }}"
                                                                             width="120" height="120"
                                                                             class="rounded shadow img_border">
                                                                    </div>
                                                                @endif
                                                            </div>
                                ...
                                <div class="col-md-12 col-12 mt-3">
                                    <div class="card">
                                        <div class="card-header bg-success text-white fw-bold">
                                            در فیلد زیر میتوانید یه عکس با کیفیت برای محصول آپلود کنید
                                        </div>
                                        <div class="card-body bg-light">
                                            <div class="row">
                                                @php
                                                    $image5 = $product->images->where('position',5)->first();
                                                @endphp
                                                <div class="col-12 col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <div id="preview5" class="mt-2 d-flex flex-wrap gap-3">
                                                            @if($image5)
                                                                <div class="preview-item d-flex align-items-center gap-2" id="image-{{ $image5->id }}">
                                                                    <img src="{{ url('get_image_by_id/'.$image5->id) }}" width="120" class="rounded shadow">
                                ...
                                <div class="col-12 border-top border-bottom py-3 mb-3">
                                    <div id="attribute-container">
                                            ...
                                            @foreach($old_meta_keys as $index => $oldKey)
                                                <div class="row attribute-row mb-2 meta-row">
                                                    <div class="col-6 col-md-3">
                                                        ...
                                                        <span
                                                            class="alert_required bg-danger text-white p-1 rounded d-inline-block mt-1"><strong>{{ $message }}</strong></span>
                                                        @enderror
                                                    </div>
                                        ...
                                        @elseif(isset($product_Meta) && $product_Meta->count() > 0)

                                            @foreach($product_Meta as $index => $meta)
                                                <div class="row attribute-row mb-2 meta-row">
                                                    <div class="col-6 col-md-3">
                                                        <select name="meta_key[]" class="form-control form-control-sm">
                                                            <option value="">انتخاب ویژگی</option>
                                                            @foreach($attributes as $attribute)
                                                                <option value="{{ $attribute->id }}"
                                                                    {{ $meta->meta_key == $attribute->id ? 'selected' : '' }}>
                                                                    {{ $attribute->attribute }}
                                                                </option>
                                                            @endforeach
                                                        </select>
...
@section('script')
    <script src="{{asset('admin-assets/jquery/jquery-image-upload-resizer-complete.js')}}"></script>
    <script>
        $(document).ready(function () {
            load_articles();

            $('#product_name_id_in_app').prop('disabled', true);
            $('#product_group_id_in_app').prop('disabled', true);
        ...
        function add_articles(dataArray) {
            ...
            $select.empty().append(new Option("نام کالا را انتخاب کنید...", ""));
            ...
            let selectedValue = "{{ old('product_name_id_in_app', $product->product_id_in_app) }}";
            let selectedText = "{{ old('product_name', $product->product_name) }}";
