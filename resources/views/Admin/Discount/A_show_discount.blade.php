            ...
            <div class="col-12">
                <section class="main-body-container">
                    ...
                    <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                        <a href="{{route('A_create_discount')}}" class="btn btn-info btn-sm">ایجاد عنوان تخفیف</a>
                    </section>
                        ...
                        <table id="producttable" class="table table-striped table-hover h-150px">
                            ...
                            @foreach ($discounts as $discount)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $discount->discount_type == 'amazingsale' ? 'فروش شگفت انگیز' : 'عمومی'}} </td>
                                    <td>{{ $discount->start_date }} </td>
                                    <td>{{ $discount->end_date }} </td>
                                    <td>{{ $discount->status == 1 ? 'فعال ' :  'غیر غعال'}} </td>
                                    ...
                                    <td class="width-8-rem text-center">
                                        ...
                                        <form action="{{ route('A_inactive_discount', $discount->id) }}"
                                              method="POST"
