...
<div class="verify-box w-100" style="max-width: 420px; margin-top: 10rem;">
    ...
    <!-- Form -->
    <form action="{{ route('s_mobile_number_verification') }}" method="POST">
        @csrf
    ...
    <div class="text-center mt-4 small text-muted">
        <p id="timerText">ارسال مجدد کد تا <span id="timer" class="fw-bold text-purple">1:00</span></p>
        <p id="resendText" class="d-none">
            کد را دریافت نکردید؟
            <button id="resendBtn" type="button" class="btn btn-link p-0 fw-semibold text-purple text-decoration-underline custom-cl-primary">
                ارسال مجدد
            </button>
        </p>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $resendBtn.on('click', function() {
            ...
            $.ajax({
                url: '{{ route('resend_verification_code') }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    username: '{{ $username }}'
